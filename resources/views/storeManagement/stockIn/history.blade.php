    ...
    <div class="card shadow ">
        <div class="card-header">
            ...
            <div class="card-tools">
                <form action="{{ route(Auth::user()->role->name . '.inventory.item.stock_in_history') }}" method="GET">
                    @csrf
                    <div class="input-group input-group-sm">
                        ...
                        <div class="input-group-append">
                            ...
                            <a
                                href="{{ route(Auth::user()->role->name . '.inventory.item.stock_in_history') }}"class="btn btn-sm mt-1 mb-1 btn-primary  ml-2"><i
                                    class="fa fa-plus" aria-hidden="true"></i> stock_in</a>
                            <a
                                href="{{ route(Auth::user()->role->name . '.inventory.item.stock_in_history') }}"class="btn btn-sm mt-1 mb-1 btn-default  ml-2">All
                                History</a>
                        </div>
                ...
                <tbody>
                    @foreach ($datas as $data)
                        ...
                        <td class="text-center">
                            {{-- info --}}
                            <button class="btn btn-sm mt-1 mb-1 btn-info" onclick="stockInhistory(this,{{ $data->id }})"><i
...
@section('scripts')
    ...
    <script>
        ...
        function stockInhistory(button, id) {
            setTimeout(() => {
            ...
            $.ajax({
                url: "{{ route(Auth::user()->role->name . '.inventory.item.stock_in_history_info') }}",
                method: 'GET',
                // dataType: 'json',
                data: {
                    id
                ...
                success: function(datas) {
                    ...
                    if (datas.length !== 0) {
                        ...
                        resultItem += `
                                    <tr class="text-sm bg-warning">
                                        <td></td>
                                        <td colspan="3">Total Price : ${total_price}<span class="text-xs">/\=</span> </td>
                                    </tr>`;
