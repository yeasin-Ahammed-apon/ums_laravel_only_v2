    ...
    <div class="card shadow ">
        <div class="card-header">
            ...
            <div class="card-tools">
                <form action="{{ route(Auth::user()->role->name . '.inventory.item.stock_return_history') }}" method="GET">
                    @csrf
                    <div class="input-group input-group-sm">
                        ...
                        <div class="input-group-append">
                            ...
                            <a
                                href="{{ route(Auth::user()->role->name . '.inventory.item.stock_return_history') }}"class="btn btn-sm mt-1 mb-1 btn-primary  ml-2"><i
                                    class="fa fa-plus" aria-hidden="true"></i> stock_in</a>
                            <a
                                href="{{ route(Auth::user()->role->name . '.inventory.item.stock_return_history') }}"class="btn btn-sm mt-1 mb-1 btn-default  ml-2">All
                                History</a>
                        </div>
...
@section('scripts')
    ...
    <script>
        ...
        function stockOuthistory(button, id) {
            ...
            $.ajax({
                url: "{{ route(Auth::user()->role->name . '.inventory.item.stock_return_history_info') }}",
                method: 'GET',
                // dataType: 'json',
                data: {
                    id
