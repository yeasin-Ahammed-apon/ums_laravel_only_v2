    ...
    <div class="card shadow ">
        <div class="card-header">
            ...
            <div class="card-tools">
                <form action="{{ route(Auth::user()->role->name . '.inventory.categorie.item') }}" method="GET">
                    @csrf
                    <div class="input-group input-group-sm">
                        ...
                        <div class="input-group-append">
                            ...
                            <a
                                href="{{ route(Auth::user()->role->name . '.inventory.categorie.item') }}"class="btn btn-sm mt-1 mb-1 btn-default  ml-2">All
                                Categories</a>
                            <a
                                href="{{ route(Auth::user()->role->name . '.inventory.categorie.item', ['status' => 1]) }}"class="btn btn-sm mt-1 mb-1 btn-success mr-2 ml-2">Active
                                Categories</a>
                            <a href="{{ route(Auth::user()->role->name . '.inventory.categorie.item', ['status' => 0]) }}"
        ...
        <table class="table  table-bordered border-top">
            ...
            <tbody>
                @foreach ($datas as $data)
                    <td>{{ $data->name }}</td>
                    <td class="p-1 bg-info text-sm" style="border: 2px solid black">
                        <ul class="p-0">
                            @foreach ($data->inventory_item as $item)
