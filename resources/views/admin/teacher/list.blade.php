                ...
                <form action="{{ route('admin.teacher.index') }}" method="GET">
                    ...
                    <div class="input-group input-group-sm">
                        ...
                        <div class="input-group-append">
                            ...
                            <a href="{{ route('admin.teacher.create') }}"class="btn btn-primary  ml-2">+ Add Teacher</a>
                            <a href="{{ route('admin.teacher.index') }}"class="btn btn-default  ml-2">All Teacher</a>
                            <a
                                href="{{ route('admin.teacher.index', ['status' => 1]) }}"class="btn btn-success mr-2 ml-2">Active
                                Teacher</a>
                ...
                @foreach ($datas as $data)
                        ...
                        <td>
                            @if ($data->user->status === 1)
                                <a href="{{ route('admin.teacher.status', $data->user->id) }}" class="btn btn-outline-success">Active</a>
                            @else
                                <a href="{{ route('admin.teacher.status', $data->user->id) }}" class="btn btn-outline-warning">Deactive</a>
                            @endif
                        </td>
                        <td class="text-center">
                            <a href="{{ route('admin.teacher.show', $data->id) }}" class="btn btn-success">View</a>
                            <a href="{{ route('admin.teacher.edit', $data->id) }}" class="btn btn-primary">Edit</a>
