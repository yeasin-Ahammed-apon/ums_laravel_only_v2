        ...
        <div class="card bg-light d-flex flex-fill">
          ...
          <div class="card-footer">
            <div class="text-right">
                <a href="{{ route(Auth::user()->role->name.'.admin.edit', $data->id) }}" class="btn btn-sm mt-1 mb-1 btn-primary"><i class="fa fa-cogs" aria-hidden="true"></i> Edit</a>
                <a href="{{ route(Auth::user()->role->name.'.page.settings', $data->user_id) }}" class="btn btn-sm mt-1 mb-1 btn-primary">User Page Settings</a>
                <form action="{{ route(Auth::user()->role->name.'.admin.destroy', $data->id) }}" method="POST"
                    class="d-inline">
                    @csrf @method('DELETE')
