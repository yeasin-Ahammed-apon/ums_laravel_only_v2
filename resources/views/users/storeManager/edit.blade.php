...
@section('content')
    ...
    <div class="card">
        <form action="{{ route(Auth::user()->role->name.'.storeManager.update', $data->user_id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="card-body row">
                {{-- role --}}
                <input type="hidden" name="role" value="storeManager" class="form-control" placeholder="Enter name">
                {{-- name --}}
            ...
            <div class="card-footer">
                ...
                <a href="{{ route(Auth::user()->role->name.'.storeManager.show', $data->id) }}" class="btn btn-sm mt-1 mb-1 btn-success"><i class="fa fa-eye" aria-hidden="true"></i> View</a>
            </div>
        </form>
    </div>
    <!-- /.card -->
