    ...
    <div class="card shadow ">
        <div class="card-header">
            ...
            <div class="card-tools">
                <form action="{{ route(Auth::user()->role->name . '.library.categorie.index') }}" method="GET">
                    @csrf
                    <div class="input-group input-group-sm">
                        ...
                        <div class="input-group-append">
                            ...
                            <a
                                href="{{ route(Auth::user()->role->name . '.library.categorie.create') }}"class="btn btn-sm mt-1 mb-1 btn-primary  ml-2"><i
                                    class="fa fa-plus" aria-hidden="true"></i> Add Categories</a>
                            <a
                                href="{{ route(Auth::user()->role->name . '.library.categorie.index') }}"class="btn btn-sm mt-1 mb-1 btn-default  ml-2">All
                                Categories</a>
                            <a
                                href="{{ route(Auth::user()->role->name . '.library.categorie.index', ['status' => 1]) }}"class="btn btn-sm mt-1 mb-1 btn-success mr-2 ml-2">Active
                                Categories</a>
                            <a href="{{ route(Auth::user()->role->name . '.library.categorie.index', ['status' => 0]) }}"
        ...
        <table class="table  table-bordered border-top">
            ...
            <tbody>
                @foreach ($datas as $data)
                    ...
                    <td>
                        @if ($data->status)
                            <a href="{{ route(Auth::user()->role->name . '.library.categorie.status', $data->id) }}"
                    ...
                    <td class="text-center">
                        {{-- view --}}
                        <a href="{{ route(Auth::user()->role->name . '.library.categorie.show', $data->id) }}"
                            class="btn btn-sm mt-1 mb-1 btn-info"><i class="fas fa-info-circle "></i> </a>
                        {{-- edit --}}
                        <a href="{{ route(Auth::user()->role->name . '.library.categorie.edit', $data->id) }}"
                            class="btn btn-sm mt-1 mb-1 btn-primary edit"><i class="fa fa-cogs" aria-hidden="true"></i></a>
                        {{-- delete --}}
                        <form action="{{ route(Auth::user()->role->name . '.library.categorie.destroy', $data->id) }}"
